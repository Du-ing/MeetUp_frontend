  ...
  <div class="table-classic-wrapper">
    <el-card shadow="always">
      ...
      >
        ...
        <el-table-column prop="masterId" label="标题编号" align="center" />
        <el-table-column label="标题" align="center">{{titleName}}</el-table-column>
        <el-table-column prop="detailId" label="编号" align="center"  sortable />
        <el-table-column label="图片" align="center">
          <template slot-scope="scope">
            <el-image style="width: 100px; height: 100px" :src="scope.row.pictureUrl" fit="fill" />
          </template>
        </el-table-column>
...
export default {
  ...
  created() {
    ...
    this.titleId = this.$route.params.data.titleId
  ...
  methods: {
    ...
    handleDelete(data) {
      ...
      this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
        ...
        service({
          url: "/peach/detail/deleteTitleByIds",
          method: "post",
          data: data
        }).then(() => {
          this.$message({
    ...
    fetchData() {
      ...
      // 获取数据列表接口
      service({
        url: '/peach/detail/getDetailByTitleId',
        method: 'get',
        params: {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if(this.dialogFormTitle == "编辑"){
            url = "/aaa"
            data.append("titleId", this.dialogForm.titleId)
          }
          data.append("name", this.dialogForm.name)
          service({
            url: url,
            method: "post",
            data: data
    ...
    exportTable(type) {
      if (this.tableData.length) {
        const params = {
          header: ['编号', '图片', '描述'],
          key: ['detailId', 'pictureUrl', 'description'],
          data: this.tableData,
          autoWidth: true,
          fileName: '导出表格',
          bookType: type

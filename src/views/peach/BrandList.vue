  ...
  <div class="table-classic-wrapper">
    <el-card shadow="always">
      ...
      >
        ...
        <el-table-column prop="brandId" label="编号" align="center" sortable />
        <el-table-column prop="name" label="名称" align="center" />
        <el-table-column label="内容" align="center">
          <el-button slot-scope="scope" type="success" @click="handleEdit(scope.row)" round>编辑内容</el-button>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="200">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="danger" @click="handleDelete([scope.row.brandId])">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页栏 -->
  ...
  methods: {
    ...
    handleDelete(data) {
      ...
      this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
        ...
        service({
          url: "/peach/brand/deleteBrandByIds",
          method: "post",
          data: data
        }).then(() => {
          this.$message({
    ...
    searchData() {
      ...
      // 获取数据列表接口
      service({
        url: '/peach/brand/selectBrandLike',
        method: 'get',
        params: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.formVisible = false
          let url = "/peach/brand/addBrand"
          let data = new FormData()
          if(this.dialogFormTitle == "编辑"){
            url = "/aaa"
            data.append("brandId", this.dialogForm.brandId)
          }
